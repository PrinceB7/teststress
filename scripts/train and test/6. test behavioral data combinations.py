...
combinations = [el for el in os.listdir(settings.behavioral_features_combinations_dir) if '.' not in el]
...
for moments_string in combinations:
    for participant in settings.participants:
        ...
        params, scores = utils.participant_train_test_xgboost(participant=participant, train_dir=settings.behavioral_features_dataset_dir, selected_features=utils.get_behavior_enhanced_selected_features(moments_string=moments_string), tuning=True)
    ...
    with open(f"{settings.behavioral_data_combinations_test_results_dir}/{moments_string}_scores.csv", "w+") as w_scores, open(f"{settings.behavioral_data_combinations_test_results_dir}/{moments_string}_params.csv", "w+") as w_params:
