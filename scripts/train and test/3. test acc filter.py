...
for participant in settings.participants:
    ...
    try:
        for params, scores in utils.participant_train_test_xgboost(participant=participant, train_dir=settings.acc_filtered_dataset_dir, m=settings.m, model_dir=settings.acc_filtered_model_dir):
...
with open(f"{settings.acc_filter_test_results_dir}/scores.csv", "w+") as w_scores:
    with open(f"{settings.acc_filter_test_results_dir}/params.csv", "w+") as w_params:
